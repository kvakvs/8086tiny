...
#define R_M_OP(dest, op, src)                                               \
    (i_w ? op_dest = CAST(unsigned short) dest,                             \
     op_result =                                                            \
         ...
         : (op_dest = dest,                                                 \
            op_result = dest op(op_source = CAST(uint8_t) src)))
...
uint8_t mem[RAM_SIZE], io_ports[IO_PORT_COUNT], *opcode_stream, *regs8,
...
// instructions into a much smaller number of distinct functions, which we then
// execute
                       ...
                       : MUL_MACRO(uint8_t, regs8) OPCODE 5
                             :  // IMUL
                                ...
                                : MUL_MACRO(int8_t, regs8) OPCODE 6
                                      :  // DIV
                                         i_w
                                         ? DIV_MACRO(unsigned short, unsigned,
                                                     regs16)
                                         : DIV_MACRO(uint8_t,
                                                     unsigned short, regs8)
                                                   OPCODE 7
                                               :  // IDIV
                                                  i_w
                                                  ? DIV_MACRO(short, int,
                                                              regs16)
                                                  : DIV_MACRO(int8_t, short,
                                                              regs8);
            }
