    ...
    for (; opcode_stream = mem + 16 * regs16[REG_CS] + reg_ip,
        ...
        switch (xlat_opcode_id) {
            OPCODE_CHAIN 0 :  // Conditional jump (JAE, JNAE, etc.)
                ...
                reg_ip +=
                        (int8_t)i_data0 *
                        (i_w ^ (regs8[bios_table_lookup[TABLE_COND_JUMP_DECODE_A]
                        [scratch_uchar]] ||
                                regs8[bios_table_lookup[TABLE_COND_JUMP_DECODE_B]
                                [scratch_uchar]] ||
                                regs8[bios_table_lookup[TABLE_COND_JUMP_DECODE_C]
                                [scratch_uchar]] ^
                ...
                :  // POP regs16
                R_M_POP(regs16[i_reg4bit]) OPCODE 2
                :  // INC|DEC regs16
                i_w = 1;
                i_d = 0;
                i_reg = i_reg4bit;
                DECODE_RM_REG;
                i_reg =
                        extra OPCODE_CHAIN 5 :                  // INC|DEC|JMP|CALL|PUSH
                if (i_reg < 2)  // INC|DEC
                    ...
                    R_M_PUSH(regs16[REG_CS]),  // CALL (far)
                            ...
                            i_reg & 1 && (regs16[REG_CS] = CAST(int16_t)
                ...
                switch (i_reg) {
                    OPCODE_CHAIN 0 :                     // TEST
                        ...
                        : MUL_MACRO(uint8_t, regs8) OPCODE 5
                        :  // IMUL
                        i_w
                        ? MUL_MACRO(int16_t, regs16)
                        : MUL_MACRO(int8_t, regs8) OPCODE 6
                        :  // DIV
                        i_w
                        ? DIV_MACRO(uint16_t, unsigned,
                                    regs16)
                        : DIV_MACRO(uint8_t,
                                    uint16_t, regs8)
                    OPCODE 7
                        :  // IDIV
                        i_w
                        ? DIV_MACRO(int16_t, int,
                                    regs16)
                        : DIV_MACRO(int8_t, int16_t,
                                    regs8);
                }
                ...
                switch (extra) {
                    OPCODE_CHAIN 0
                        ...
                        :  // SBB
                        ADC_SBB_MACRO(-) OPCODE 4
                        :  // AND
                        OP(&=) OPCODE 5
                        :  // SUB
                        OP(-=),
                                set_CF(op_result > op_dest) OPCODE 6
                        :  // XOR
                        OP(^=) OPCODE 7
                        :  // CMP
                        OP(-),
                                set_CF(op_result > op_dest) OPCODE 8
                        :  // MOV
                        OP(=);
                }
                ...
                switch (i_reg) {
                    OPCODE_CHAIN 0
                        ...
                        scratch2_uint &= (1 << scratch_uint) - 1,
                                R_M_OP(mem[rm_addr], +=, scratch2_uint
                        ...
                        set_OF(SIGN_OF(op_result) ^
                               set_CF(scratch2_uint & 1 << (TOP_BIT - scratch_uint)))
                        OPCODE 3
                        :  // RCR
                        R_M_OP(mem[rm_addr],
                    ...
                    OPCODE 5 :  // SHR
                        set_OF(SIGN_OF(op_dest)) OPCODE 7
                        :  // SAR
                ...
                switch (i_reg4bit) {
                    OPCODE_CHAIN 0 :  // LOOPNZ
                        scratch_uint &=
                                !regs8[FLAG_ZF] OPCODE 1 :  // LOOPZ
                        scratch_uint &=
                                regs8[FLAG_ZF] OPCODE 3
                        :  // JCXXZ
                        scratch_uint = !++regs16[REG_CX];
                }
                reg_ip += scratch_uint * (int8_t)i_data0 OPCODE 14
                :  // JMP | CALL int16_t/near
                reg_ip += 3 - i_d;
                if (!i_w) {
                    if (i_d)  // JMP far
            ...
            OPCODE 16 :  // XCHG AX, regs16
                ...
                if (extra & 2)  // IRET
                    ...
                    regs16[REG_SP] += i_data0 OPCODE 20
                :  // MOV r/m, immed
                R_M_OP(mem[op_from_addr], =, i_data2) OPCODE 21
                :                       // IN AL/AX, DX/imm8
                ...
                (io_ports[0x3D5] =
                         ((mem[0x49E] * 80 + mem[0x49D] + CAST(int16_t) mem[0x4AD]) &
                ...
                scratch_uint =
                        extra ? regs16[REG_DX] : (uint8_t)i_data0;
                ...
                rep_override_en = 2;
                rep_mode = i_w;
                seg_override_en&& seg_override_en++ OPCODE 25
                :  // PUSH reg
                R_M_PUSH(regs16[extra]) OPCODE 26
                :  // POP reg
                R_M_POP(regs16[extra]) OPCODE 27
                ...
                :  // CBW
                regs8[REG_AH] =
                        -SIGN_OF(regs8[REG_AL]) OPCODE 31
                :  // CWD
                regs16[REG_DX] =
                        -SIGN_OF(regs16[REG_AX]) OPCODE 32
                ...
                OPCODE 34 :  // POPF
                set_flags(R_M_POP(scratch_uint)) OPCODE 35
                :  // SAHF
                make_flags();
                ...
                regs16[REG_AX] = op_result =
                        0xFF &
                        regs8[REG_AL] +
                        i_data0 * regs8[REG_AH] OPCODE 43 :  // SALC
                regs8[REG_AL] =
                        -regs8[FLAG_CF] OPCODE 44 :  // XLAT
                regs8[REG_AL] =
                        mem[SEGREG(seg_override_en ? seg_override : REG_DS,
                                   REG_BX, regs8[REG_AL] +)] OPCODE 45
                :  // CMC
                regs8[FLAG_CF] ^= 1 OPCODE 46
...
void CPU::audio_callback(void *data, uint8_t *stream, int len) {
    ...
    for (int i = 0; i < len; i++)
        stream[i] =
                (self->spkr_en == 3) && CAST(uint16_t) self->mem[0x4AA]
                ? -((54 * self->wave_counter++ / CAST(uint16_t) self->mem[0x4AA]) & 1)
